...
select a.pad,b.pad,count(*) from (select pad,count(id) t from sharding_4_t1 group by pad having t>1)a join (select pad,count(id) t from schema2.sharding_4_t2 group by pad order by pad)b group by a.pad,b.pad order by a.pad,b.pad;
...
drop table if exists sharding_4_t1;
CREATE TABLE sharding_4_t1(`id` int(10) unsigned NOT NULL,`k` int(10) unsigned NOT NULL DEFAULT '0',`c` char(120),`pad` int(11) NOT NULL,PRIMARY KEY (`id`),UNIQUE KEY (`k`))CHARACTER SET = utf8mb4;
...
select pad,count(id) t from sharding_4_t1 group by pad having t>1 order by pad;/*allow_diff_sequence*/
...
select * from sharding_4_t1 where pad>(select count(id) t from sharding_4_t1 group by pad,id having t>3 order by id) order by id;
...
select * from (select pad,count(id) t from sharding_4_t1 group by pad having t>1 order by pad)a where a.pad>1;/*allow_diff_sequence*/
...
select (select count(id) t from sharding_4_t1 group by pad,id having t>3 order by id) from sharding_4_t1;
